...
const FrequencyInfo: React.FC<Props> = ({frequencyInfo, mood, onBack}) => {
  ...
  const handleViewMood = useCallback(() => {
    if (!mood) {
      showToast('Mood data unavailable for this frequency.', 'info');
      return;
    }

    navigation.navigate(
      routes.MOOD as keyof RootStackParamList,
      {data: mood},
    );
  }, [mood, navigation]);
  ...
  const handleShare = async () => {
    try {
      ...
      const shareUrl =
        (frequencyInfo as any).share_url ||
        frequencyInfo.background_image ||
        '';
      ...
      <View style={styles.frequencyInfoWrapper}>
        ...
        <ScrollView
          ...
          <Text style={styles.frequencyInfoDescription}>
            {mood?.description ?? frequencyInfo.detailed_information}
          </Text>
        </ScrollView>
        ...
        <View style={styles.actionButtonContainer}>
          <TouchableOpacity
            style={[
              ...
              !mood && styles.actionButtonDisabled,
            ]}
            disabled={!mood}
            onPress={handleViewMood}>
            <Text style={styles.actionButtonText}>View all Grounding</Text>
          </TouchableOpacity>
