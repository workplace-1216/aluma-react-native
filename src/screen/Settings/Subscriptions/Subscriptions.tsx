...
const Subscriptions = () => {
  ...
  const [selectedPlan, setSelectedPlan] = useState<SubscriptionPlan | null>(
    user?.subscription?.plan === 'free' || !user?.subscription ? 'monthly' : null,
  );
  ...
  const handleContinue = async () => {
    if (!selectedPlan) {
      showToast('Select a plan to continue.', 'info');
      return;
    }
                ...
                <TouchableOpacity
                  ...
                  <Text
                    style={[
                      ...
                      splitPrice(plan.price)[1]
                        ? planStyles.planPriceWithSecondary
                        : null,
                      isSelected ? planStyles.selectedText : planStyles.unselectedText,
                    ]}>
                    {splitPrice(plan.price)[0]}
                  </Text>
                  {splitPrice(plan.price)[1] ? (
                    <Text
                      style={[
                        planStyles.planPriceSecondary,
                        isSelected ? planStyles.selectedText : planStyles.unselectedText,
                      ]}>
                      ({splitPrice(plan.price)[1]})
                    </Text>
                  ...
                  {isCurrent ? (
                    <Text
                      style={[
                        ...
                        isSelected && styles.currentPlanNoteSelected,
                      ]}>
                      Current plan
                    </Text>
                  ) : null}
          ...
          <View style={styles.buttonContainer}>
            ...
            {normalizedCurrentPlan !== 'free' && (
              <TouchableOpacity
                ...
                disabled={isLoading}>
                <Text style={styles.restoreButtonText}>Cancel subscription</Text>
              </TouchableOpacity>
            )}
